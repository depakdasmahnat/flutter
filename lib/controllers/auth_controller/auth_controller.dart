  ...
  logOut({
    ...
    try {
      await clearUserData(context).then((val) {
        ...
        showSnackBar(
            context: context,
            text: message ?? 'Successfully Logout',
    ...
    await loadingDialog(
      ...
      if (response != null) {
        ...
        if (responseData?.status == true) {
          ...
          showSnackBar(
              context: context, text: 'Something went wong', color: Colors.red);
        }
      }
    });
    // return responseData;
      ...
      await ApiService()
        ...
        if (response != null) {
          ...
          Fetchinterestquestions responseData =
              Fetchinterestquestions.fromJson(json);
          if (responseData.status == true) {
            fetchinterestquestions = responseData;
            if (fetchinterestquestions?.data?.isNotEmpty == true) {
              ...
              questionId2 =
                  fetchinterestquestions?.data?[1].id.toString() ?? '';
