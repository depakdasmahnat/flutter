...
class _VerifyResetPasswordOTPState extends State<VerifyResetPasswordOTP> {
  ...
  late String? mobileNo = widget.contact;
  ...
  @override
  void initState() {
    print('check first name ${widget.enagicId}');

    print('check mobileNo name ${widget.contact}');
  ...
  Future verifyOtp({required String otp}) async {
    if (otp.length == totalOtpFields) {
      await context.read<MemberAuthControllers>().verifyResetPasswordOtp(
            context: context,
            enagicId: widget.enagicId,
            contact: widget.contact,
            otp: otp,
          );
      setState(() {});
    } else {
      showError(context: context, message: 'Wrong Password');
    }
  }
}
